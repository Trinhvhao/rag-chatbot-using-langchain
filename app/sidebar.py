...
def display_sidebar():
    ...
    model_options = ["gemini-2.5-flash", "gemini-2.5-pro"]  # ✅ update model list
    ...
    if uploaded_file is not None:
        if st.sidebar.button("Upload"):
            with st.spinner("Uploading..."):
                ...
                if upload_response:
                    st.sidebar.success(
                        f"File '{uploaded_file.name}' uploaded successfully with ID {upload_response.get('file_id')}."
                    )
                    # Refresh the list after upload
                    st.session_state.documents = list_documents() or []
        ...
        if st.sidebar.button("Delete Selected Document"):
            with st.spinner("Deleting..."):
                ...
                if delete_response:
                    ...
                    st.session_state.documents = list_documents() or []  # Refresh after deletion
                else:
